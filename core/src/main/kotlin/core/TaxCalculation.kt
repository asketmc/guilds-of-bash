...
fun calculateTaxAmount(guildRankOrdinal: Int, baseAmount: Int): Int {
    ...
    // Use `entries` (Kotlin 1.9) instead of `values()` to avoid warnings and
    // avoid calling the reflection-backed array creation multiple times.
    val entries = Rank.entries
    val idx = (guildRankOrdinal - 1).coerceIn(0, entries.lastIndex)
    val rank = entries[idx]
