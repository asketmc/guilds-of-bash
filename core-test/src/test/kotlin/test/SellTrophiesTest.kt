 ...
 */
@P2
class SellTrophiesTest {

    @Test
    fun `sell all sells all trophies when amount is 0`() {
        val state = stateWithEconomy(moneyCopper = 100, trophiesStock = 5, reservedCopper = 0)
        val rng = Rng(100L)
    ...
    fun `sell partial sells exact amount`() {
        val state = stateWithEconomy(moneyCopper = 0, trophiesStock = 10, reservedCopper = 0)
        val rng = Rng(100L)
    ...
    fun `sell amount greater than stock is rejected by validation`() {
        val state = stateWithEconomy(moneyCopper = 0, trophiesStock = 2, reservedCopper = 0)
        val cmd = SellTrophies(amount = 3, cmdId = 1L)
    ...
    fun `sell with negative amount is allowed and sells all`() {
        val state = stateWithEconomy(moneyCopper = 50, trophiesStock = 8, reservedCopper = 0)
        val rng = Rng(100L)
    ...
    fun `sell when stock is zero emits no events`() {
        val state = stateWithEconomy(moneyCopper = 100, trophiesStock = 0, reservedCopper = 0)
        val rng = Rng(100L)
        val cmd = SellTrophies(amount = 0, cmdId = 1L)
    ...
    fun `sell exact stock amount works`() {
        val state = stateWithEconomy(moneyCopper = 20, trophiesStock = 4, reservedCopper = 0)
        val rng = Rng(100L)
    ...
    fun `sell more than stock sells only available stock`() {
        val state = stateWithEconomy(moneyCopper = 10, trophiesStock = 3, reservedCopper = 0)
        val rng = Rng(100L)
    ...
    fun `validation accepts zero amount`() {
        val state = stateWithEconomy(moneyCopper = 0, trophiesStock = 5, reservedCopper = 0)
        val cmd = SellTrophies(amount = 0, cmdId = 1L)
    ...
    fun `sell increments revision`() {
        val state = stateWithEconomy(moneyCopper = 0, trophiesStock = 1, reservedCopper = 0)
        val rng = Rng(100L)
        val cmd = SellTrophies(amount = 1, cmdId = 1L)
